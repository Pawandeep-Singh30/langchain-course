# Chnage 1: Add re + inspect - we'll parse tool calls rom raw text instead of structured json
import re
import inspect
...
# CHnage 3: Delete the JSON schemas. Tools now live inside the prompt as plain tex.
# we derive descriptions from the functions themselves using inspect.

def get_tool_descriptions(tools_dict):
    descriptions = []
    for tool_name, tool_function in tools_dict.items():
        #__wrapped__bypassess decorator wrappers (e.g., @traceable adds *, config = None)
...
Answer the following question as best as you can. You can access to the following tools:
...
Action Input: the input to the action, as commma separated values
Observation: the result of the action
... (this Thought/Action/Action Input/Observation ca repeat N times)
...
def run_agent(question: str):
    ...
    # Change 5: One prompt strng replaces the system/user message split
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        # Change 6 : Parse tool calls from raw text with regex - fragile if LLM doesnt follow format
        ...
        # CHnage 7: History is one growing string re-sent every iteation (replaces messages.append).
